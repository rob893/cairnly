# Security Review — Cairnly Template
...
- **Location(s):** `Cairnly.API/appsettings.json:43-49` (`OpenApi.Enabled=true`, `AuthSettings.RequireAuth=false`); `Cairnly.API/Middleware/OpenApiBasicAuthMiddleware.cs:39-43`; `Cairnly.API/ApplicationStartup/ApplicationBuilderExtensions/OpenApiApplicationBuilderExtensions.cs:25-52`
...
- **Location(s):** `Cairnly.API/Data/Repositories/UserRepository.cs:135-140` (`CheckPasswordSignInAsync(user, password, false)`); `Cairnly.API/ApplicationStartup/ServiceCollectionExtensions/IdentityServiceCollectionExtensions.cs:15-22` (no `opt.Lockout`); `Cairnly.API/Controllers/V1/AuthController.cs:137-142`
...
- **Location(s):** `Cairnly.API/Controllers/V1/AuthController.cs:189-214` (Google link path); `:206` (`EmailConfirmed || validatedToken.EmailVerified`); `:308-335` (GitHub link path)
...
- **Location(s):** `Cairnly.API/Controllers/V1/AuthController.cs:129-142` (early return before password check); `:84-87` (register returns Identity `DuplicateUserName`/`DuplicateEmail` descriptions)
...
- **Location(s):** `Cairnly.API/ApplicationStartup/ApplicationBuilderExtensions/CorsApplicationBuilderExtensions.cs:10-22`
...
- **Location(s):** `Cairnly.API/Middleware/OpenApiBasicAuthMiddleware.cs:54-55` (`Split(':',2)[1]`); `:77-81` (`string.Equals` comparisons)
...
- **Severity / Priority:** Low / 3.0
- **Impact / Risk / Effort:** 1 / 2 / 1
- **Location(s):** `Cairnly.API/Controllers/V1/AuthController.cs:504-510` (`csrfHeader != csrfCookie`); `:563` (`Guid.NewGuid().ToString()` CSRF token)
...
- **Location(s):** `Cairnly.API/Middleware/PathBaseRewriterMiddleware.cs:26-29`; pipeline order `Cairnly.API/ApplicationStartup/Startup.cs:68-69`
...
- **Location(s):** `Cairnly.API/Controllers/V1/AuthController.cs:398-415` (`GitHubCallback` passes `state` through, never validates); `:425-442` (`GoogleCallback`); `:166-258`/`:280-388` (`login/google`, `login/github` accept a raw `code` with no state binding); `Cairnly.API/Services/Auth/GitHubOAuthService.cs:44-60` (token exchange omits `redirect_uri`)
...
- **Location(s):** `Cairnly.API/ApplicationStartup/ServiceCollectionExtensions/IdentityServiceCollectionExtensions.cs:15-22` (no `RequireConfirmedEmail`, `RequiredLength = 8`); `Cairnly.API/Controllers/V1/AuthController.cs:122-153` (login does not check `EmailConfirmed`)
...
- **Severity / Priority:** Low / 2.0
- **Impact / Risk / Effort:** 2 / 2 / 2
- **Location(s):** `Cairnly.API/ApplicationStartup/ServiceCollectionExtensions/AuthenticationServiceCollectionExtensions.cs:48` (signing key built with no length check); `Cairnly.API/Services/Auth/JwtTokenService.cs:127-132` (length check only on the issue path); `Cairnly.API/appsettings.Local.example.json:7` (`<your-jwt-signing-secret-min-32-chars>`)
...
- **Seeded admin (`admin@cairnly.local`) is created with no password** (`DatabaseSeeder.cs:138-157`) — not directly loginable, but a known default identity; ensure it is removed/renamed for real deployments.
...
- Frontend (`cairnly-ui/**`) token storage, axios CSRF/refresh handling, and XSS were out of scope for this backend+infra pass.
...
- **Location(s):** cairnly-ui/src/components/ApiErrorDisplay.tsx:41-66; consumed with `showDetails={true}` in cairnly-ui/src/pages/LoginPage.tsx:63, RegisterPage.tsx:88, ForgotPasswordPage.tsx:74, ResetPasswordPage.tsx:122
...
- **Severity / Priority:** Low / 4.0
- **Impact / Risk / Effort:** 2 / 2 / 1
- **Location(s):** cairnly-ui/src/services/axiosConfig.ts:17-20
...
- **Location(s):** cairnly-ui/.env (tracked by git — confirmed via `git ls-files`); cairnly-ui/.gitignore:6 only ignores `.env.local`
...
- **Severity / Priority:** Low / 3.0
- **Impact / Risk / Effort:** 2 / 1 / 1
- **Location(s):** cairnly-ui/package.json:51 (vite → esbuild); `npm audit` reports GHSA-g7r4-m6w7-qqqr
...
- **Severity / Priority:** Medium / 2.5
- **Impact / Risk / Effort:** 3 / 2 / 2
- **Location(s):** cairnly-ui/src/utils/oauthUtils.ts:86-97 (reads `window.location.hash`); cairnly-ui/src/pages/OAuthCallbackPage.tsx:47-97 (processes but never clears the hash)
...
- **Severity / Priority:** Medium / 2.0
- **Impact / Risk / Effort:** 3 / 3 / 3
- **Location(s):** cairnly-ui/index.html:1-15 (no CSP `<meta>`); no header-based CSP in scope
...
- **Location(s):** cairnly-ui/src/services/auth.ts:8,24-30,44-78; read in cairnly-ui/src/services/axiosConfig.ts:24-27
...
- **Location(s):** cairnly-ui/src/utils/passwordValidation.ts:6-25
...
- **Location(s):** cairnly-ui/src/hooks/api.ts:47-67 (`useHelloV1`/`useHelloV2` use `enabled: !isAuthLoading`); contrast notes hooks which use `enabled: isAuthenticated && !isAuthLoading` (api.ts:70-79,122-131)
...
- **Location(s):** cairnly-ui/src/utils/oauthUtils.ts:62-84 (authorize request builds `client_id`, `redirect_uri`, `scope`, `state` — no `code_challenge`)
