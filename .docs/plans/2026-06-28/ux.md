...
| --- | --------------------------------------------------------- | ------ | ------ | ------------ |
| 1   | No success feedback or live-region after mutations        | High   | Medium | ✅ Done (W2) |
| 2   | Forgot/Reset password bypass branded AuthShell            | High   | Medium | ✅ Done (W2) |
...
- **What was done (W2):** Added a global `ToastProvider` near the QueryClient in `main.tsx` plus a `utils/notifications.ts` helper, and fire calm `role="status"`/`aria-live` success toasts on `mutateAsync` resolve across transactions, accounts, balance reconcile, spending plans, and profile/preferences — saves are now confirmed visually and to screen readers.
...
- **What was done (W2):** Refactored `ForgotPasswordPage`/`ResetPasswordPage` onto the branded `AuthShell`, swapped emoji for lucide icons, and replaced all undefined v2 tokens with theme tokens — the recovery flow now matches Login/Register in dark mode.
...
- **What was done (W1):** Swapped the ❌/⚠️/✓ emoji in `ApiErrorDisplay`/`ErrorBoundary`/`ProfileSection` for lucide `AlertCircle`/`TriangleAlert`/`CheckCircle2` and replaced dead v2 tokens with `text-muted`/`to-surface`/`text-accent` — error/verify states now match the lucide+theme system.
...
- **What was done (W1):** Re-tokenized `DateField` to `rounded-[var(--field-radius)] bg-[var(--field-background)]` so date inputs are pixel-consistent with HeroUI text inputs in shared modals.
...
- **What was done (W1):** Added a shared `components/EmptyState.tsx` (icon + title + subtitle + optional CTA) and adopted it in Transactions (with an "Add transaction" CTA), Home, SpendingPlans, and Accounts — first-run is consistent and guided.
