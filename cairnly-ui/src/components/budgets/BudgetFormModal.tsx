...
interface BudgetFormModalProps {
  ...
  /** The budget being edited, or `undefined` when creating a new one. */
  budget?: Budget;
  onSubmit(payload: CreateBudgetRequest): Promise<unknown>;
  isPending: boolean;
  error: Error | null;
}

/** Modal form for creating or editing a budget's name, currency, and description. */
export function BudgetFormModal({ isOpen, onOpenChange, budget, onSubmit, isPending, error }: BudgetFormModalProps) {
      ...
      <Modal.Backdrop>
        <Modal.Container size="md">
          <Modal.Dialog>
            <Modal.CloseTrigger />
            <Modal.Header>
              <Modal.Heading>{isEdit ? 'Edit budget' : 'New budget'}</Modal.Heading>
            </Modal.Header>
