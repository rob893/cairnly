...
import { cadenceLabel, incomeTypeLabel } from '../../types/budgets';
import type {
  BudgetExpense,
  BudgetIncome,
  CadenceAmounts,
...
type LineItem = BudgetIncome | BudgetExpense;
...
/**
 * Lists a budget's income or expense line items in a HeroUI table with built-in
 ...
 */
export function LineItemsSection({ kind, budgetId, currency }: LineItemsSectionProps) {
  const isIncome = kind === 'income';
  const title = isIncome ? 'Income' : 'Expenses';
  const addLabel = isIncome ? 'Add income' : 'Add expense';

  const incomesQuery = useBudgetIncomes(isIncome ? budgetId : undefined);
  const expensesQuery = useBudgetExpenses(isIncome ? undefined : budgetId);
  ...
  const handleSubmit = async (payload: CreateBudgetIncomeRequest & CreateBudgetExpenseRequest) => {
    const expensePayload: CreateBudgetExpenseRequest = {
      name: payload.name,
    ...
    cells.push(
      <Table.Cell key="actions">
        <div className="flex items-center justify-end gap-1">
          <Button variant="ghost" size="sm" onPress={() => openEdit(item)} aria-label={`Edit ${item.name}`}>
            Edit
          </Button>
          <Button variant="danger-soft" size="sm" onPress={() => setDeleteTarget(item)} aria-label={`Delete ${item.name}`}>
            Delete
          </Button>
        </div>
      </Table.Cell>
      ...
      <CardContent className="px-2 pb-4 sm:px-6 sm:pb-6">
        {query.isLoading ? (
          ...
          <p className="py-8 text-center text-sm text-muted">
            No {kind} line items yet. Add one to start building this budget.
          </p>
        ) : (
