...
/** A spendingPlan summarized for the dashboard list at the selected cadence (minor units). */
export interface SpendingPlanListItem {
  id: number;
  name: string;
  currency: string;
  income: number;
...
interface SpendingPlansListCardProps {
  items: SpendingPlanListItem[];
  isLoading: boolean;
}

/**
 * A scannable list of spendingPlans, each with an income-vs-expenses progress bar and
 * remaining amount, linking through to the spendingPlan detail page.
 */
export function SpendingPlansListCard({ items, isLoading }: SpendingPlansListCardProps) {
  return (
    <Card className="bg-surface border border-border">
      <CardHeader className="flex items-center justify-between gap-3 px-6 pt-6">
        <h2 className="text-lg font-semibold">Spending Plans</h2>
        <Link to="/spending-plans" className="text-sm font-medium text-accent no-underline hover:opacity-80">
          View all →
        </Link>
      </CardHeader>
      <CardContent className="px-6 pb-6">
        {isLoading ? (
          ...
          <p className="py-6 text-center text-sm text-muted">No spending plans to show.</p>
        ) : (
